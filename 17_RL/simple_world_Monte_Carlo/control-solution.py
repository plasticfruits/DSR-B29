...
## 1.7 create an array for values of all states
...
# 1.3 create the environment
env = Maze(N=N, wall_length=wall_length, seed=np.random.randint(low=1, high=10_000))
...
for ctrl_episode_id in range(nr_ctrl_episodes):
    ...
    for eval_episode_id in tqdm(
        range(nr_eval_episodes),
        "evaluation " + str(ctrl_episode_id) + "/" + str(nr_ctrl_episodes),
    ):
    ...
    plotter(ax, V, vmax=0, vmin=-4.0 * N, env=env)
    ...
    pi = pi + epsilon * np.exp(-1.0 * ctrl_episode_id / epsilon_decay_window)
